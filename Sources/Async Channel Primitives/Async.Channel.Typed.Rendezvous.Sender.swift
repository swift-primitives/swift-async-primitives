...
// ===----------------------------------------------------------------------===//
    ...
    extension Async.Channel.Typed.Rendezvous.Sender where Element: ~Copyable, Failure: Swift.Error & Sendable {
        ...
        public func send(_ element: consuming sending Element) async -> Send.Outcome {
            ...
            let signal: Signal = await withTaskCancellationHandler {
                await unsafe withUnsafeContinuation { (raw: UnsafeContinuation<Signal, Never>) in
                    ...
                    var cancelled = Deque<Column.Ring<Async.Channel<Element>.Typed<Failure>.Rendezvous.Receiver.Waiter>>()
                    ...
                    if cancelledSelf {
                        ...
                        waiter.continuation.take()?.resume(returning: .rejected(Error(terminal: terminal)))
                    }
                    if let receiver {
        ...
        func terminate(_ terminal: Async.Channel<Element>.Typed<Failure>.Terminal) {
            var receivers = Deque<Column.Ring<Async.Channel<Element>.Typed<Failure>.Rendezvous.Receiver.Waiter>>()
