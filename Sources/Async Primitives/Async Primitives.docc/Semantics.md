...
| ``Async/Channel/Typed/Bounded`` | `send(_:)` and `receive()` preserve `.cancelled` while suspended. `finish()` crosses direction as successful termination; `fail(_:)` crosses direction as the exact declared `Failure`. A sender failure follows FIFO buffered drain before `receive()` throws `.failed(_)`; a receiver failure rejects subsequent and suspended senders with that same `.failed(_)` | Inherits the bounded channel's mutex-acquisition ordering and per-sender FIFO | Capacity-bounded; typed `send(_:)` suspends and resumes through the same backpressure queue | Inherits the bounded channel's FIFO suspended-sender wakeup discipline |
| ``Async/Channel/Duplex`` | Each peer owns a typed inbound receiver and outbound sender. A terminal operation affects only its direction: outbound finish or failure drains at the peer inbound, and inbound failure rejects the matching peer outbound | Each direction independently inherits typed bounded FIFO ordering | Two independently capacity-bounded directions | Each direction independently inherits typed bounded suspended-sender FIFO wakeup |
